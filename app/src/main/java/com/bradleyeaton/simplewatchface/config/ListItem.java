...
public class ListItem {
    ...
    public ListItem(Context context, String label, int imageType, int tintColor, Intent intent, int requestCode) {
        ...
        this.foreground = new ScaleDrawable(foregroundUnscaled, 0, FOREGROUND_DIAMETER, FOREGROUND_DIAMETER).getDrawable().mutate();
    ...
    private void setForegroundColor(){
        ...
        if(hsl[2] > 0.8){
            //Too light background then switch foreground to a dark color
            foreground.setColorFilter(new PorterDuffColorFilter(Color.BLACK, PorterDuff.Mode.MULTIPLY));
        } else {
            if(foreground.getColorFilter() != null){
